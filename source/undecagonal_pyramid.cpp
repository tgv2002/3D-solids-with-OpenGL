...
GLfloat UndecagonalPyramid::v_undec_pyr_top_vertices[99];
GLfloat UndecagonalPyramid::v_undec_pyr_bottom_vertices[99];
GLfloat UndecagonalPyramid::c_undec_pyr_top_vertices[99];
GLfloat UndecagonalPyramid::c_undec_pyr_bottom_vertices[99];

UndecagonalPyramid::UndecagonalPyramid(float x, float y) {
    this->position = glm::vec3(x, y, 0.0f);
    this->rotation = 52 + 90;
    this->is_spinning = 0;
    speed = 1;

    // undecagonal pyramid base polygon
    getRegularPolygon(0.0f, 0.0f, 0.0f, 0.45f, 11, 1.0f, 0.5f, 0.0f);
    for(int j=0;j<198;j++){
        this->undec_pyr_bottom_vertices[j] = this->triangleArrayP[j];
    }

    // undecagonal pyramid top faces
    getTrianglesOnPolygon(0.0f, 0.70f, 0.0f, 11);
    for(int j=0;j<198;j++){
        this->undec_pyr_top_vertices[j] = this->triangleArray[j];
    }

    splitVerticesAndColors();
    this->object1 = create3DObject(GL_TRIANGLES, 33, this->v_undec_pyr_top_vertices, this->c_undec_pyr_top_vertices, GL_FILL);
    this->object2 = create3DObject(GL_TRIANGLES, 33, this->v_undec_pyr_bottom_vertices, this->c_undec_pyr_bottom_vertices, GL_FILL);
}

void UndecagonalPyramid::draw(glm::mat4 VP) {
    Matrices.model = glm::mat4(1.0f);
    glm::mat4 translate = glm::translate (this->position);    // glTranslatef
    glm::mat4 rotate    = glm::rotate((float)(52.0 * M_PI / 180.0f), glm::vec3(1, 0, 0));
    if(this->is_spinning)
...
void UndecagonalPyramid::set_position(float x, float y, float z) {
    this->position = glm::vec3(x, y, z);
}

void UndecagonalPyramid::set_spin_status(int should_spin){
...
void UndecagonalPyramid::getColorsList(int groupSize){
...
void UndecagonalPyramid::makePolygonWithTriangles(float centre_x, float centre_y,
...
void UndecagonalPyramid::getRegularPolygon(float centre_x, float centre_y, float centre_z, 
...
// Obtain 11 triangles on top of polygon based on external vertex and polygon vertices
void UndecagonalPyramid::getTrianglesOnPolygon(float ext_x, float ext_y, float ext_z, int polygonVertices){
    getColorsList(3);
...
void UndecagonalPyramid::splitVerticesAndColors(){
    int v_ctr = 0;
    int c_ctr = 0;

    for(int i=0;i<198;i++){
        if(i % 6 < 3)
            this->v_undec_pyr_top_vertices[v_ctr++] = this->undec_pyr_top_vertices[i];
        else
            this->c_undec_pyr_top_vertices[c_ctr++] = this->undec_pyr_top_vertices[i];
    }
    ...
    for(int i=0;i<198;i++){
        if(i % 6 < 3)
            this->v_undec_pyr_bottom_vertices[v_ctr++] = this->undec_pyr_bottom_vertices[i];
        else
            this->c_undec_pyr_bottom_vertices[c_ctr++] = this->undec_pyr_bottom_vertices[i];
    }
}